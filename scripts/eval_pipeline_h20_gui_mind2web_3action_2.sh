...
# 遍历ckpt编号，从1到10为例
ckpt_numbers=(100 150 200 250 300 350)
for ckpt_num in "${ckpt_numbers[@]}"; do
    ...
    python inference/inference_vllm_mind2web_guir1_3action.py \
        --model_path $LOCAL_HF_DIR \
        --data_path ${DATA_DIR}/metadata/hf_test_full.json \
        --image_dir ${DATA_DIR}/images \
        --output_name ${SAVE_NAME}_global_step_${ckpt_num}_3action \
        --num_actor 2 \
        --use_history \
        --history_num 4
    python eval/eval_mind2web_reformat.py \
        --pred_path $OUTPUT_DIR/${SAVE_NAME}_global_step_${ckpt_num}_3action/hf_test_full.json \
