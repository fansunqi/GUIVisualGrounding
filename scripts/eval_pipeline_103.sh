...
SAVE_NAME=mind2web_gt_sim_0_9_grpo_qwen2_5_vl_3b

export TORCH_COMPILE_CACHE=/mnt/Shared_06_disk1/fsq/vllm_cache

# 遍历ckpt编号，从1到10为例
ckpt_numbers=(200 250 300 325 350 375)
for ckpt_num in "${ckpt_numbers[@]}"; do
    echo "Processing ckpt number: $ckpt_num"

    LOCAL_DIR=/mnt/Shared_06_disk1/fsq/gui-r1_exp/$SAVE_NAME/global_step_$ckpt_num/actor
    ...
    python inference/inference_vllm_mind2web.py \
        ...
        --data_path /mnt/Shared_06_disk1/fsq/data/Mind2Web/metadata/hf_test_full.json \
    ...
    python eval/eval_mind2web_reformat.py \
        --pred_path $OUTPUT_DIR/${SAVE_NAME}_global_step_$ckpt_num/hf_test_full.json \
        --gt_path /mnt/Shared_06_disk1/fsq/data/Mind2Web/metadata/hf_test_full.json
    cd ..
    cd scripts  
done
