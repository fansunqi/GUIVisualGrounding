...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text=(
            ...
            "Please provide the action to perform (enumerate from ['scroll', 'click', 'complete']), the point where the cursor is moved to (integer) if a click is performed, and any input text required to complete the action.\n"
            "Output the thinking process in <think> </think> tags, and the final answer in <answer> </answer> tags as follows:\n"
            "<think> ... </think> <answer>[{'action': enum['scroll', 'click', 'complete'], 'point': [x, y], 'input_text': 'no input text [default]'}]</answer>\n"
